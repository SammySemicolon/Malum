...
public class MalumVanillaRecipes implements IConditionBuilder {
    ...
    protected static void buildRecipes(RecipeOutput output) {
        ...
        shaped(RecipeCategory.MISC, ItemRegistry.SPIRIT_POUCH.get()).define('X', Tags.Items.STRINGS).define('Y', ItemRegistry.SOULWOVEN_SILK.get()).define('Z', ItemTags.SOUL_FIRE_BASE_BLOCKS).pattern(" X ").pattern("YZY").pattern(" Y ").unlockedBy("has_spirit_fabric", has(ItemRegistry.SOULWOVEN_SILK.get())).save(output);
        ...
        //BANNERS
        shaped(RecipeCategory.MISC, ItemRegistry.SOULWOVEN_BANNER.get()).define('X', ItemTagRegistry.RUNEWOOD_PLANKS).define('Y', ItemRegistry.SOULWOVEN_SILK.get()).pattern("X").pattern("Y").pattern("Y").unlockedBy("has_soulwoven_silk", has(ItemRegistry.SOULWOVEN_SILK.get())).save(output);
